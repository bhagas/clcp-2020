...
router.get('/', cek_login, function(req, res) {
  connection.query("SELECT * from pupz ", function(err, rows, fields) {
    if (err) throw err;
            // console.log(rows);
 res.render('content-backoffice/manajemen_pupz/list', { data : rows});
})
 
});

router.get('/insert', cek_login, function(req, res) {
      res.render('content-backoffice/manajemen_pupz/insert');
});

router.get('/edit/:id', cek_login, function(req, res) {
  connection.query("SELECT * from pupz where id="+req.params.id, function(err, rows, fields) {
    if (err) throw err;
            // console.log(rows);
 res.render('content-backoffice/manajemen_pupz/edit', { data : rows});
})
...
//  console.log(post)
  ...
  sql_enak("pupz").where("id", req.body.id)
  .update(post).then(function (count) {
 console.log(count);
})
.finally(function() {
  //sql_enak.destroy();
  res.redirect('/manajemen_pupz'); 
});
});
